...
const updateHeader = function (status) {
  const lastError = JSON.parse(
    sessionStorage.getItem("verification_session_error") || null
  );

  const title_header = document.querySelector(".status-title");
  title_header.textContent = "You are not verified yet.";
  ...
  fetch("/pushToToken", {
      ...
      body: {"test":"test"}
      // body: {"id": localStorage.getItem("currentUserId")},
    }).then((res)=>console.log(res));

    console.log("nextstep:" + localStorage.getItem("currentUserId"));

    title_header.textContent = "You can now apply!";
    document.querySelector(".status-note").textContent =
      "For this demo, we will successfully verify everyone";
  }
